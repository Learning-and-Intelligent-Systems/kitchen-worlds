(define (stream fe-gripper-tamp)
  ...
  (:stream inverse-kinematics-wconf
    ...
    :outputs (?q ?t)
    :certified (and (SEConf ?q) (Traj ?t) (KinWConf ?a ?o ?p ?g ?q ?t ?w))
  )
  (:stream plan-free-motion-wconf
    :inputs (?q1 ?q2 ?w)
  ...
  (:stream get-joint-position-open
    ...
    (:stream inverse-kinematics-grasp-handle
      :inputs (?a ?o ?p ?g ?w)
      :domain (and (Controllable ?a) (Position ?o ?p) (HandleGrasp ?o ?g) (IsClosedPosition ?o ?p) (WConf ?w))
      :outputs (?q ?t)
      :certified (and (SEConf ?q) (Traj ?t) (KinGraspHandle ?a ?o ?p ?g ?q ?t ?w))
    )
