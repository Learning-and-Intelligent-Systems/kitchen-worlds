(define (stream pr2-tamp)
  ...
  (:stream inverse-kinematics
    ...
    :outputs (?t)
    :certified (and (ATraj ?t) (Kin ?a ?o ?p ?g ?q ?t))
  )

  (:stream plan-base-motion
    :inputs (?q1 ?q2)
    :domain (and (BConf ?q1) (BConf ?q2))
    :fluents (AtPose AtPosition AtAConf) ; AtBConf AtAConf ; AtGrasp
    :outputs (?t)
    :certified (and (BTraj ?t) (BaseMotion ?q1 ?t ?q2))
  )

  ;(:stream plan-base-motion-with-obj
  ;  :inputs (?q1 ?q2 ?a ?o ?g)
  ;  :domain (and (BConf ?q1) (BConf ?q2) (AtGrasp ?a ?o ?g))
  ;  :fluents (AtPose AtGrasp AtPosition AtAConf)
  ;  :outputs (?t)
  ;  :certified (and (BTraj ?t) (BaseMotionWithObj ?q1 ?t ?a ?o ?g ?q2))
  ;)
