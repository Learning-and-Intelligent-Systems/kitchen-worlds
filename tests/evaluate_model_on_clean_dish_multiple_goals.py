...
def goal_actions_in_sequence(goal_actions, action_sequence):
    # check if the list of goal actions is in the action sequence
    # goal_actions: N
    # action_sequence: T
    # return: bool
    success = True
    for goal_action in goal_actions:
        if goal_action not in action_sequence:
            success = False
            break
    return success
...
def plan_multi_step(model,
    ...
    goal_action_idxs = [query_actions.index(goal_action) for goal_action in goal_actions]
    ...
    for t in range(planning_horizon):
        ...
        # filter out actions that have already been executed (e.g., should not pick the same object from the same location twice)
        # concept idx order: action, color, object, location
        # query_actions_scores: B, K
        # sampled_action_idx_sequences: B, T
        # query_actions_concept_idxs: K, L
        # next_action_concept_idxs: B, T, L

        # for bi in range(sampled_action_idx_sequences.shape[0]):
        #     sampled_action_idx_sequence = sampled_action_idx_sequences[bi]  # T
        #     for ai in sampled_action_idx_sequence:
        #         query_actions_scores[bi, ai] = 0.0

        # this part assigns a zero score to (action, color, object) that is already in the sampled sequence
        if avoid_repeating_object_pick_place:
            if t > 0:
                for bi in range(next_action_concept_idxs.shape[0]):
                    # print("bi", bi)
                    # print("query_actions_scores", query_actions_scores.shape, query_actions_scores)
                    next_action_concept_idx = next_action_concept_idxs[bi]  # T, L
                    next_aco_idx = next_action_concept_idx[:, :3]  # T, 3 (action, object, location)
                    # print("next_aco_idx", next_aco_idx.shape, next_aco_idx)
                    # print("query_actions_concept_idxs", query_actions_concept_idxs[:, :3].shape, query_actions_concept_idxs[:, :3])
                    # print("------")
                    for qai, query_action_concept_idx in enumerate(query_actions_concept_idxs):
                        # debug here. next_aco_idx is T, 3, query_action_concept_idx is 3
                        # if np.all(next_aco_idx == query_action_concept_idx[:3]):
                        if any((next_aco_idx[:] == query_action_concept_idx[:3]).all(1)):
                        # if query_action_concept_idx in next_aco_idx.tolist():
                            # print(query_action_concept_idx[:3])
                            # print(query_actions_scores[bi, qai])
                            query_actions_scores[bi, qai] = 0.0
                    # print("query_actions_scores", query_actions_scores.shape, query_actions_scores)
        ...
        for bi in range(len(sampled_action_idx_sequences)):
            ...
            if goal_actions_in_sequence(goal_action_idxs, action_idx_sequence):
                print("\n" + "*"*20)
                print(f"plan for goals {goal_actions} found!")
                ...
                print("*" * 20 + "\n")
                texts.append(f"plan for goals {goal_actions} found!")
...
def run_high_level_policy(env: CleanDishEnvV1, exp_dir, max_depth=5, debug=True, only_consider_feasible=True):
    ...
    for t in range(max_depth):
        ...
        if t == 0:
            before_texts = [f"Goal actions: {goal_actions} | Goal concept actions: {goal_concept_actions}"]
        else:
            before_texts = []
        after_texts = []
        ...
        goal_concept_action_sequence, log_img_text_pair = plan_multi_step(model,
                        ...
                        query_actions, goal_concept_actions,
                        num_obj_pts, num_scene_pts, device,
                        observation_mask=observation_mask,
                        admissible_concept_actions=admissible_concept_actions, debug=True,
                        action_score_threshold=0.3, planning_horizon=6, max_beam_size=100,
                                                                          avoid_repeating_object_pick_place=True)
