...
def random_rollouts(env, max_depth=6, max_rollouts=10, debug=False, simple_data=True):
    ...
    for ri in range(max_rollouts):
        ...
        depth = 0
        object_name_to_pose = env.get_object_name_to_pose()
        next_node = (obs, commands_so_far, current_g, current_gp, symbolic_state, action_to_feasibility, object_name_to_pose, depth)
        next_action = None

        while True:

            (cur_obs, commands_so_far, current_g, current_gp, symbolic_state, action_to_feasibility, object_name_to_pose, depth) = next_node
            if debug:
                print("\n\n" + "=" * 100)
                ...
                print("depth: ", depth)
                print("object_name_to_pose: ", object_name_to_pose)
                # plt.imshow(cur_obs.rgbPixels)
                # plt.show()
            ...
            for text_action in symbolic_feasible_text_actions:
                ...
                new_object_name_to_pose = env.get_object_name_to_pose()
                if new_score == -1:
                    # not feasible
                    action_to_feasibility[text_action] = -1
                else:
            ...
            if simple_data:
                ...
                rollout.append((cur_obs, symbolic_state, current_g, current_gp, action_to_feasibility, object_name_to_pose, depth, next_action))
            else:
                rollout.append((cur_obs, commands_so_far, current_g, current_gp, symbolic_state, action_to_feasibility, object_name_to_pose, depth, next_action))
    ...
    if debug:
        for ri, rollout in enumerate(rollouts):
            ...
            for d in rollout:
                print("\n" + "-" * 50)
                if simple_data:
                    cur_obs, symbolic_state, current_g, current_gp, action_to_feasibility, object_name_to_pose, depth, next_action = d
                    print(d)
                else:
                    cur_obs, commands_so_far, current_g, current_gp, symbolic_state, action_to_feasibility, object_name_to_pose, depth, next_action = d
                # print(cur_obs.rgbPixels)
                # img = Image.fromarray(cur_obs.rgbPixels, 'RGBA')
                # img.show()
                plot_images({view_name: Image.fromarray(cur_obs[view_name][0], 'RGBA') for view_name in cur_obs})
