...
def main(parallel=True):
    ...
    if parallel:
        ...
        #bad_indices = [0, 6]
        #bad_indices = [0, 1]
        # if '17' not in cases[index]: continue
        indices = set(range(args.index, num_cases)) - set(bad_indices)
        print('Indices:', indices)
        for i, index in enumerate(indices):
            print(f'{i+1}/{len(indices)}) | Index: {index} | Case: {cases[index]}')
            process(cases[index], parallel=False)
